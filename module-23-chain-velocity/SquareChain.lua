...
--[[
    Increases the chain's velocity by reducing the structure's refresh
    rate by a specified momentum percentage.

    Ensures the refresh rate isn't so low that the game becomes
    unplayable.
]]
function SquareChain:increaseVelocity(momentumPercentage)
    self.secPerUpdate = math.max(MIN_REFRESH_RATE, self.secPerUpdate - self.secPerUpdate * momentumPercentage)
end
